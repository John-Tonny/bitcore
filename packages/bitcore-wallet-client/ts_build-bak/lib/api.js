...
var API = (function (_super) {
    ...
    function API(opts) {
        ...
        _this.request = new request_1.Request(opts.baseUrl || BASE_URL, {
            r: opts.request,
    ...
    API.prototype._processTxps = function (txps) {
        ...
        lodash_1.default.each([].concat(txps), function (txp) {
            ...
            txp.message = common_1.Utils.decryptMessageNoThrow(txp.message, encryptingKey) || null;
    ...
    API.prototype.fromObj = function (credentials) {
    ...
    API._buildSecret = function (walletId, walletPrivKey, coin, network) {
        ...
        return lodash_1.default.padEnd(widBase58, 22, '0') + walletPrivKey.toWIF() + (network == 'testnet' ? 'T' : 'L') + coin;
    };
    ...
    API.prototype._addSignaturesToBitcoreTxBitcoin = function (txp, t, signatures, xpub) {
        $.checkState(txp.coin);
        ...
        var i = 0;
        var x = new bitcore.HDPublicKey(xpub);
    ...
    API.prototype._addSignaturesToBitcoreTx = function (txp, t, signatures, xpub) {
        var coin = txp.coin, network = txp.network;
        var chain = common_1.Utils.getChain(coin);
        switch (chain) {
            case 'XRP':
            case 'ETH':
                var unsignedTxs = t.uncheckedSerialize();
                var signedTxs_1 = [];
                for (var index = 0; index < signatures.length; index++) {
                    var signed = CWC.Transactions.applySignature({
                        chain: chain,
                        tx: unsignedTxs[index],
                        signature: signatures[index]
                    });
                    signedTxs_1.push(signed);
                    t.id = CWC.Transactions.getHash({ tx: signed, chain: chain, network: network });
                }
                t.uncheckedSerialize = function () { return signedTxs_1; };
                t.serialize = function () { return signedTxs_1; };
                break;
            default:
    ...
    API.prototype.joinWallet = function (secret, copayerName, opts, cb) {
        ...
        this._doJoinWallet(secretData.walletId, secretData.walletPrivKey, this.credentials.xPubKey, this.credentials.requestPubKey, copayerName, {
            ...
            if (err)
                return cb(err);
            if (!opts.dryRun) {
                _this.credentials.addWalletInfo(wallet.id, wallet.name, wallet.m, wallet.n, copayerName, {
    ...
    API.prototype.getNotifications = function (opts, cb) {
        ...
        this.request.getWithLogin(url, function (err, result) {
            ...
            var notifications = lodash_1.default.filter(result, function (notification) {
                return opts.includeOwn || notification.creatorId != _this.credentials.copayerId;
            });
    ...
    API.prototype.getCoinsForTx = function (opts, cb) {
        $.checkState(this.credentials && this.credentials.isComplete());
        opts = opts || {};
        var url = '/v1/txcoins/';
        url +=
    ...
    API.prototype._getCreateTxProposalArgs = function (opts) {
        ...
        lodash_1.default.each(args.outputs, function (o) {
            o.message = API._encryptMessage(o.message, _this.credentials.sharedEncryptingKey) || null;
        });
        return args;
    };
    API.prototype.createTxProposal = function (opts, cb, baseUrl) {
        var _this = this;
        $.checkState(this.credentials && this.credentials.isComplete());
    ...
    API.prototype.createRedeemTxProposal = function (opts, cb, baseUrl) {
        var _this = this;
        $.checkState(this.credentials && this.credentials.isComplete());
        $.checkState(this.credentials.sharedEncryptingKey);
        $.checkArgument(opts);
    ...
    API.prototype.createAtomicSwapTxProposal = function (opts, cb, baseUrl) {
        var _this = this;
        $.checkState(this.credentials && this.credentials.isComplete());
        $.checkState(this.credentials.sharedEncryptingKey);
        $.checkArgument(opts);
    ...
    API.prototype.getPayProV2 = function (txp) {
        if (!txp.payProUrl || this.doNotVerifyPayPro)
    ...
    API.prototype.broadcastTxProposal = function (txp, cb) {
        ...
        this.getPayProV2(txp)
            ...
            if (paypro) {
                ...
                var chain_1 = common_1.Utils.getChain(txp.coin);
                var currency_1 = txp.coin.toUpperCase();
                var rawTxUnsigned = t_unsigned.uncheckedSerialize();
                var serializedTx = t.serialize({
                    disableSmallFees: true,
                    disableLargeFees: true,
                    disableDustOutputs: true
                });
                var unsignedTransactions = [];
                var signedTransactions_1 = [];
                ...
                for (var _i = 0, unserializedTxs_1 = unserializedTxs; _i < unserializedTxs_1.length; _i++) {
                    ...
                    if (isBtcSegwit) {
                        var unsignedSize = unsigned.length / 2;
                ...
                for (var _a = 0, serializedTxs_1 = serializedTxs; _a < serializedTxs_1.length; _a++) {
                    var signed = serializedTxs_1[_a];
                    signedTransactions_1.push({
                        tx: signed,
    ...
    API.prototype.addAccess = function (opts, cb) {
        ...
        var encCopayerName = opts.name ? common_1.Utils.encryptMessage(opts.name, this.credentials.sharedEncryptingKey) : null;
        var opts2 = {
            copayerId: copayerId,
            requestPubKey: requestPubKey,
    ...
    API.upgradeCredentialsV1 = function (x) {
        $.shouldBeObject(x);
        if (!lodash_1.default.isUndefined(x.version) || (!x.xPrivKey && !x.xPrivKeyEncrypted && !x.xPubKey)) {
    ...
    API.serverAssistedImport = function (opts, clientOpts, callback) {
        ...
        var checkCredentials = function (key, opts, icb) {
            ...
            var client = clientOpts.clientFactory ? clientOpts.clientFactory() : new API(clientOpts);
            client.fromString(c);
            client.openWallet({}, function (err, status) {
                if (!err) {
                    ...
                    var clients_1 = [client];
                    var tokenAddresses = status.preferences.tokenAddresses;
                    if (!lodash_1.default.isEmpty(tokenAddresses)) {
                        lodash_1.default.each(tokenAddresses, function (t) {
                            ...
                            var tokenCredentials = client.credentials.getTokenCredentials(token);
                            var tokenClient = lodash_1.default.cloneDeep(client);
        ...
        var checkKey = function (key, cb) {
            ...
            async.eachSeries(opts, function (x, next) {
                ...
                checkCredentials(key, optsObj, function (err, iclients) {
                    if (err)
                        return next(err);
    ...
    API.prototype.simplexGetQuote = function (data) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            _this.request.post('/v1/service/simplex/quote', data, function (err, data) {
                if (err)
                    return reject(err);
                return resolve(data);
            });
    ...
    API.prototype.simplexPaymentRequest = function (data) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            _this.request.post('/v1/service/simplex/paymentRequest', data, function (err, data) {
                if (err)
                    return reject(err);
                return resolve(data);
            });
    ...
    API.prototype.simplexGetEvents = function (data) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            var qs = [];
            qs.push('env=' + data.env);
            _this.request.get('/v1/service/simplex/events/?' + qs.join('&'), function (err, data) {
                if (err)
                    return reject(err);
                return resolve(data);
            });
